...
function afficherClient ($client){
    ...
    function modifierClient($client,$id){
        $sql="UPDATE client SET id=:idd, nom=:nom,prenom=:prenom,email=:email, pwd=:pwd, adresse=:adresse,numero=:numero WHERE id=:idd";
...
try{        
 ...
 function rechercherClient($id){
        ...
        $sql="SELECT * from client where id LIKE "%'.$q.'%" ORDER BY id DESC";
        $db = config::getConnexion();
        try{
        $liste=$db->query($sql);
        return $liste;
